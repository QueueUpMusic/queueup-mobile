...
export default function NewRoundScreen() {
  ...
  useEffect(() => { if (!edit) return; void getStaffRounds().then(({ rounds }) => { const round = rounds.find((item) => item.id === Number(edit)); if (round) { setSeasonId(round.season_id); setPrompt(round.prompt); setDetails(round.details); setGoesLiveAt(round.goes_live_at ?? ''); setSubmissionOpens(round.submission_opens); setSubmissionDeadline(round.submission_deadline); setVotingDeadline(round.voting_deadline); setRevealAt(round.reveal_at); } }).catch(() => setError(ApiError.networkError('Unable to load the round for editing.'))); }, [edit]);
  ...
  const save = useCallback(async (saveAction: 'draft' | 'publish') => {
    ...
    try {
      await saveStaffRound({ season: seasonId, prompt, details, goes_live_at: goesLiveAt || null, submission_opens: submissionOpens, submission_deadline: submissionDeadline, voting_deadline: votingDeadline, reveal_at: revealAt, host: null, playlist_url: null, save_action: saveAction }, edit ? Number(edit) : undefined);
      router.back();
    } catch (cause) {
