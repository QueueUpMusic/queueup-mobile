...
function Header({ onBack }: { onBack: () => void }) { const insets = useSafeAreaInsets(); return <View style={[styles.header, { paddingTop: insets.top + Spacing.sm }]}><Pressable accessibilityLabel="Go back" accessibilityRole="button" hitSlop={8} onPress={onBack} style={{ alignItems: 'center', height: 44, justifyContent: 'center', width: 44 }}><ChevronLeft color={colors.text} /></Pressable><Text style={styles.headerTitle}>Season management</Text><View style={{ width: 44 }} /></View>; }
