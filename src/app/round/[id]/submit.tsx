...
function PreviewManager({ track, onStop }: { track: SpotifySearchTrack | null; onStop: () => void }) {
  const player = useAudioPlayer(track?.preview || null);
  const status = useAudioPlayerStatus(player);

  useEffect(() => {
    if (track?.preview) void player.play();
    return () => { player.pause(); };
  }, [player, track?.id, track?.preview]);
...
function TrackRow({ track, selectedPreview, onChoose, onPreview }: { track: SpotifySearchTrack; selectedPreview: SpotifySearchTrack | null; onChoose: () => void; onPreview: () => void }) {
  ...
  return <View style={[styles.resultRow, unavailable && styles.unavailableRow]}><Artwork track={track} /><View style={styles.resultCopy}><Text numberOfLines={2} style={styles.resultTitle}>{track.title}</Text><Text numberOfLines={1} style={styles.resultArtist}>{track.artist}</Text><Text numberOfLines={1} style={styles.resultAlbum}>{track.album}</Text>{unavailable ? <Text style={styles.unavailableText}>{reason}</Text> : null}</View><View style={styles.rowActions}>{track.preview ? <Pressable accessibilityLabel={`${isPlaying ? 'Pause' : 'Preview'} ${track.title}`} accessibilityRole="button" disabled={unavailable} onPress={onPreview} style={({ pressed }) => [styles.previewButton, pressed && styles.pressed, unavailable && styles.disabled]}>{isPlaying ? <PauseIcon color={colors.brandLight} size={17} /> : <PlayIcon color={colors.brandLight} size={17} />}<Text style={styles.previewText}>{isPlaying ? 'Pause' : 'Preview'}</Text></Pressable> : null}<Pressable accessibilityLabel={`Choose ${track.title}`} accessibilityRole="button" disabled={unavailable} onPress={onChoose} style={({ pressed }) => [styles.chooseButton, pressed && styles.pressed, unavailable && styles.disabled]}><Text style={styles.chooseText}>Choose</Text></Pressable></View></View>;
}
