...
function formatPlace(place: number | undefined, tied = false) {
...
function Submitter({ result }: { result: RevealedSubmission }) {
  return <View style={styles.submitterLine}><Text style={styles.submitter}>Picked by </Text><ProfileLink displayName={result.submitter.display_name} style={styles.submitter} username={result.submitter.username} /></View>;
}

function ResultRow({ result }: { result: RevealedSubmission }) {
  return <View style={[styles.resultRow, result.place === 1 && styles.winnerRow]}><View style={styles.rankColumn}><Text numberOfLines={1} style={styles.place}>{formatPlace(result.place, result.tied)}</Text></View><Artwork track={result} /><View style={styles.trackCopy}><Text numberOfLines={2} style={styles.trackTitle}>{result.title}</Text><Text numberOfLines={1} style={styles.trackArtist}>{result.artist}</Text><Submitter result={result} /><Text numberOfLines={1} style={styles.score}>{result.average_score.toFixed(2)} ★ · {result.vote_count} votes</Text></View></View>;
}
